...
names(superhero_info)[names(superhero_info)== "Height"] <- "height"
...
names(superhero_info)[names(superhero_info)== "Alignment"] <- "alignment"
names(superhero_info)[names(superhero_info)== "Weight"] <- "weight"
```
...
## Well one of the reasons for this is that we looked at heights over 200 instead of 300.
...
```r
heavy_short <- superhero_info$name[303]
heavy_short
```
...
## 97 superheros have a combination of these three powers.
